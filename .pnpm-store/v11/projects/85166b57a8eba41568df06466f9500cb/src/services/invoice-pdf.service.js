...
function drawBrandHeader(document, fonts, order, compact = false, copyLabel = "") {
  ...
  drawText(document, fonts, compact ? `HÓA ĐƠN BÁN HÀNG · ${copyLabel}` : `HÓA ĐƠN BÁN HÀNG · ${copyLabel}`, x + 54, compact ? 55 : 61, {
    color: "#bce7dc",
    size: compact ? 7 : 7.5,
  });
...
function drawTableHeader(document, fonts, y, copyType) {
  const x = 24;
  const width = document.page.width - 48;
...
function drawItems(document, fonts, order, startY, copyType, copyLabel) {
  let y = drawTableHeader(document, fonts, startY, copyType);
  const x = 24;
  const width = document.page.width - 48;
  ...
  order.items.forEach((item, index) => {
    ...
    const detailHeight = detail
      ? document.heightOfString(detail, { width: 156, lineGap: 1 })
      : 0;
    const rowHeight = Math.max(34, 25 + detailHeight);

    if (y + rowHeight > document.page.height - 80) {
      document.addPage();
      const headerBottom = drawBrandHeader(document, fonts, order, true, copyLabel);
      y = drawTableHeader(document, fonts, headerBottom + 14, copyType);
    }
    ...
    drawText(document, fonts, item.quantity, x + 174, y + 9, {
      bold: true,
      size: 7.5,
      width: 22,
      align: "center",
...
export function renderInvoicePdf(document, order) {
  ...
  const copies = [
    { type: "customer", label: "LIÊN KHÁCH HÀNG" },
    { type: "store", label: "LIÊN CỬA HÀNG" },
  ];
  copies.forEach((copy, index) => {
    if (index > 0) document.addPage();
    const headerBottom = drawBrandHeader(document, fonts, order, false, copy.label);
    const metaBottom = drawMeta(document, fonts, order, headerBottom + 12);
    const itemsBottom = drawItems(document, fonts, order, metaBottom + 8, copy.type, copy.label);
    drawTotals(document, fonts, order, itemsBottom + 5, copy.type, copy.label);
  });
